...
//    BUTTON ------- ADC3 PB3  2|    |7  PB2 ADC1 -------- 
// NEOPIXELS ------- ADC2 PB4  3|    |6  PB1 AIN1 OC0B --- 
...
#define BT_PIN        PB3     // Pin for button
...
// NeoPixel buffer
uint8_t NEO_hue[NUM_PIXELS];
uint8_t NEO_bright[NUM_PIXELS];
...
// Clear all pixels
void NEO_clear(void) {
  for(uint8_t i=0; i<NUM_PIXELS; i++) NEO_bright[i] = 0;
}

// Fill all pixel with the same color
void NEO_fill(uint8_t hue) {
  for(uint8_t i=0; i<NUM_PIXELS; i++) NEO_hue[i] = hue;
}
...
int main(void) {
  ...
  while(1) {
    ...
    switch(state) {
      ...
      case 2:   NEO_fadeOut();
                for(uint8_t i=prng(4); i; i--) NEO_set(prng(NUM_PIXELS), prng(192));
                NEO_show();
                break;

      case 3:   for(uint8_t i=0; i<NUM_PIXELS; i++) NEO_set(i, prng(192));
                counter = 1; NEO_show();
                break;

      case 4:   for(uint8_t i=0; i<NUM_PIXELS; i++) {
                  NEO_hue[i] += prng(8); 
      ...
      case 5:   hue1 = 0;
                for(uint8_t i=0; i<NUM_PIXELS; i++, hue1+=NUM_PIXELS) NEO_set(i, hue1);
                counter = 1; NEO_show();
                break;
